 ...
 * GET /api/symbols/test-market-data/:symbol?provider=polygon
 */
 ...
 * GET /api/symbols/search?q=BTC&limit=10&providers=polygon,rithmic_websocket&categories=crypto,futures
 */
 ...
 * GET /api/symbols/market-data/:symbol?provider=polygon
 */
...
router.get('/test-analysis/:symbol', async (req, res) => {
  try {
    ...
    const technicalAnalysis = {
      ...
      rsi: Math.max(30, Math.min(70, 50 + (changePercent * 2))), // RSI based on price change
      macd: {
        value: changePercent * 0.1, // MACD based on price momentum
        signal: changePercent > 0.5 ? 'BUY' : changePercent < -0.5 ? 'SELL' : 'HOLD'
      }
    };
