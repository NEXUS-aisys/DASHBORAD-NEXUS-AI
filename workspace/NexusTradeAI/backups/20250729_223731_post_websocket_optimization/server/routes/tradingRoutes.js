...
// Yahoo Finance dependency removed - using real data providers instead
const tradingSignalService = require('../services/tradingSignalService');
const router = express.Router();

// Import DataSourceManager for real market data
let dataSourceManager = null;

// Initialize DataSourceManager (called from server.js)
  ...
  try {
    ...
    for (const symbol of portfolioSymbols) {
      try {
        ...
        // Use DataSourceManager for real market data
        if (dataSourceManager && dataSourceManager.isReady()) {
          try {
            const result = await dataSourceManager.getMarketData(symbol, null, req.user.id);
            if (result.status === 'success') {
              marketData = {
                regularMarketPrice: result.data.price,
                regularMarketPreviousClose: result.data.previousClose || result.data.price * 0.99
              };
            } else {
              throw new Error(result.error || 'No market data available');
            }
          } catch (error) {
            console.error(`Failed to get real market data for ${symbol}:`, error.message);
            // Skip this symbol instead of using fake data
            continue;
          }
        } else {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(limit, 20); i++) {
      ...
      try {
        ...
        if (dataSourceManager && dataSourceManager.isReady()) {
          try {
            const result = await dataSourceManager.getMarketData(tradeSymbol, null, req.user.id);
            if (result.status === 'success') {
              marketData = {
                regularMarketPrice: result.data.price
              };
            ...
            console.error(`Failed to get real market data for ${tradeSymbol}:`, error.message);
            // Skip this trade instead of using fake data
            continue;
          }
        } else {
          console.error('DataSourceManager not available');
...
router.get('/market/:symbol', async (req, res) => {
  try {
    ...
    // Use DataSourceManager for real market data
    if (dataSourceManager && dataSourceManager.isReady()) {
      const result = await dataSourceManager.getMarketData(symbol, null, null);
      if (result.status === 'success') {
        marketData = {
          regularMarketPrice: result.data.price,
          regularMarketPreviousClose: result.data.previousClose || result.data.price * 0.99,
          regularMarketVolume: result.data.volume || 0,
      ...
      throw new Error('DataSourceManager not available');
    }
    ...
    for (const symbol of symbols.slice(0, 4)) { // Limit to 4 recommendations
      try {
        ...
        if (dataSourceManager && dataSourceManager.isReady()) {
          ...
          console.log(`DataSourceManager not available, skipping ${symbol} recommendation`);
          continue;
        }
...
router.get('/watchlist', requireUser, async (req, res) => {
  try {
    ...
    for (let i = 0; i < Math.min(6, popularSymbols.length); i++) {
      ...
      const addedDate = new Date(Date.now() - (i * 24 * 60 * 60 * 1000)); // Sequential dates (last i days)
...
router.get('/market/providers', requireUser, async (req, res) => {
  try {
    ...
    const mockProviders = [
      { name: 'Polygon.io', enabled: true, hasApiKey: true, status: 'active' },
      { name: 'Alpha Vantage', enabled: true, hasApiKey: true, status: 'active' },
      { name: 'Bybit', enabled: true, hasApiKey: false, status: 'active' },
      { name: 'Finnhub', enabled: true, hasApiKey: true, status: 'active' },
      { name: 'IEX Cloud', enabled: false, hasApiKey: false, status: 'inactive' }
    ];
