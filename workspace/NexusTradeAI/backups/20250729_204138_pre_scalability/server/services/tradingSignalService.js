...
class TradingSignalService {
  ...
  async fetchHistoricalData(symbol, period = '3mo') {
    try {
      // Demo data since Yahoo Finance is removed - return empty array for now
      return [];
    } catch (error) {
  ...
  async getCurrentMarketData(symbol) {
    try {
      // Fixed fallback data based on real market values (no random generation)
      ...
      const change = basePrice * 0.005; // Fixed 0.5% change
      const changePercent = 0.5; // Fixed 0.5% change percentage
      
      return {
        currentPrice: basePrice,
        change: change,
        changePercent: changePercent,
        volume: 1500000, // Fixed volume
        high: basePrice * 1.02,
        low: basePrice * 0.98,
