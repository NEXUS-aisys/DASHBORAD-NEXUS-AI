...
const { dataProviderRequests, dataProviderRequestDuration } = require('./monitoringService');
...
class DataSourceManager extends EventEmitter {
  ...
  async getMarketDataWithCircuitBreaker(symbol, provider) {
    const end = dataProviderRequestDuration.startTimer({ provider: provider.name });
    try {
      ...
      if (marketData) {
        dataProviderRequests.inc({ provider: provider.name, status: 'success' });
        end();
        return {
          status: 'success',
          data: marketData,
      ...
      dataProviderRequests.inc({ provider: provider.name, status: 'failure' });
      end();
