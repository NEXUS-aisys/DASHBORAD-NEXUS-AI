...
class MockRealTimeProvider {
  ...
  async getMarketData(symbol) {
    ...
    // Add deterministic price movement based on symbol
    const symbolHash = symbol.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
    const priceVariation = ((symbolHash % 1000) / 1000 - 0.5) * 0.005; // ±0.25% variation
  ...
  generateMarketData(symbol, price, changePercent) {
    ...
    // Fixed volume based on symbol type
    const volume = symbol.includes('USDT') ? 2500000 : 1200000; // Higher volume for crypto
    const dayHigh = price * 1.025; // Fixed 2.5% above current
    const dayLow = price * 0.975; // Fixed 2.5% below current
    const open = previousClose * 1.001; // Fixed 0.1% above previous close
