...
const { register, trackHttpRequest } = require('./services/monitoringService');
...
// Clustering for scalability (ENABLED for production scale)
if (cluster.isMaster) {
  ...
  // Add metrics middleware before any routes
  ...
  */
  ...
  app.get('/api/health', cacheMiddleware(5000), (req, res) => {
    ...
    res.json({
      ...
      websocket: {
        ...
        subscriptions: socketStats.activeSubscriptions,
  ...
  // Trading routes
  const { router: tradingRoutes, initializeDataSourceManager: initializeTradingDataSourceManager } = require('./routes/tradingRoutes');
  ...
  app.get('/api/signals/test', cacheMiddleware(10000), async (req, res) => {
    try {
      ...
      for (const symbol of popularSymbols.slice(0, 6)) {
        // Get real market data using DataSourceManager with fallback
        const marketData = await getRealMarketData(symbol);
        const fallbackPrice = marketData.regularMarketPrice;
        const fallbackChange = fallbackPrice - marketData.regularMarketPreviousClose;
        const fallbackChangePercent = (fallbackChange / marketData.regularMarketPreviousClose) * 100;
  ...
  app.get('/api/portfolio/test', async (req, res) => {
    try {
      ...
      for (const symbol of portfolioSymbols) {
        ...
        const previousClose = marketData.regularMarketPreviousClose;
  ...
  app.post('/api/signals', async (req, res) => {
    ...
    try {
      ...
      socketService.broadcast('new_signal', signalData);
      ...
      for (const symbol of popularSymbols.slice(0, 6)) {
        ...
        try {
          ...
          if (symbol.endsWith('USDT')) {
            ...
            // Get real market data using DataSourceManager with fallback
            marketData = await getRealMarketData(symbol);
  ...
  app.get('/api/trading/signals/:symbol', async (req, res) => {
    try {
      ...
      // Get real market data for the symbol using DataSourceManager
      const marketData = await getRealMarketData(symbol);
      const currentPrice = marketData.regularMarketPrice;
      const previousClose = marketData.regularMarketPreviousClose || currentPrice * 0.99;
      const priceChange = currentPrice - previousClose;
      ...
      // Historical data removed - using demo data
      const historical = [];
  ...
  app.post('/api/trading/signals/batch', async (req, res) => {
    try {
      ...
      for (const symbol of symbols) {
        try {
          // Get real market data for each symbol using DataSourceManager
          const marketData = await getRealMarketData(symbol);
          const currentPrice = marketData.regularMarketPrice;
          const previousClose = marketData.regularMarketPreviousClose || currentPrice * 0.99;
          const priceChange = currentPrice - previousClose;
  ...
  app.get('/api/trading/signals/test/:symbol', async (req, res) => {
    try {
      ...
      let marketData;
      let provider = 'polygon'; // Changed from yahoo_finance to polygon
      ...
      if (symbol.endsWith('USDT')) {
        try {
          ...
          console.warn(`Bybit failed for ${symbol}, using demo data`);
      ...
      let historical = [];
      // Historical data removed - using empty array
        ...
        try {
          ...
          if (symbol.endsWith('USDT')) {
            try {
              ...
              console.warn(`Bybit failed for ${symbol}, using fallback data`);
              mappedSymbol = symbol.replace('USDT', '-USD');
              marketData = await getRealMarketData(symbol);
            }
          } else {
  ...
  app.get('/api/trading/indicators/signals/:symbol', async (req, res) => {
    try {
      ...
      // Calculate indicator signals based on demo data
      const marketData = await getRealMarketData(symbol);
      
      // Add additional market data fields needed for indicators
      if (!marketData.regularMarketDayHigh) {
        marketData.regularMarketDayHigh = marketData.regularMarketPrice * 1.02;
      }
      if (!marketData.regularMarketDayLow) {
        marketData.regularMarketDayLow = marketData.regularMarketPrice * 0.98;
      }
  ...
  // Helper function to get real market data with fallback
  async function getRealMarketData(symbol) {
    try {
      if (dataSourceManager && dataSourceManager.isInitialized) {
        const marketData = await dataSourceManager.getMarketData(symbol);
        if (marketData && marketData.price) {
          return {
            regularMarketPrice: marketData.price,
            regularMarketPreviousClose: marketData.previousClose || marketData.price * 0.99,
            regularMarketVolume: marketData.volume || 1000000,
            marketCap: marketData.marketCap || 1000000000,
            provider: marketData.provider || 'dataSourceManager'
          };
        }
      }
    } catch (error) {
      console.warn(`DataSourceManager failed for ${symbol}:`, error.message);
    }
    
    // Fallback to fixed demo data (no random)
    const basePrice = symbol === 'AAPL' ? 214.05 :
                     symbol === 'TSLA' ? 250.00 :
                     symbol === 'GOOGL' ? 175.00 :
                     symbol === 'MSFT' ? 420.00 :
                     symbol === 'AMZN' ? 185.00 :
                     symbol === 'BTC' ? 67000.00 :
                     symbol === 'ETH' ? 3400.00 : 150.00;
    
    return {
      regularMarketPrice: basePrice,
      regularMarketPreviousClose: basePrice * 0.99,
  ...
  server.listen(config.PORT, async () => {
    ...
    try {
      ...
      if (initialized) {
        ...
        // Connect the data source manager to trading routes
        initializeTradingDataSourceManager(dataSourceManager);
        console.log('✅ DataSourceManager ready - Symbol discovery and trading routes available');
        
        // Initialize Socket.IO service with DataSourceManager
        console.log('🚀 Initializing Socket.IO service...');
        const socketInitialized = await socketService.initialize(server, dataSourceManager);
        if (socketInitialized) {
          console.log('✅ Socket.IO service ready - Real-time updates enabled');
        } else {
          console.warn('⚠️ Socket.IO service initialization failed - Using fallback polling');
        }
      } else {
