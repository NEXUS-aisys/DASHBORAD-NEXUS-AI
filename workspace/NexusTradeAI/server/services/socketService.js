...
const { websocketConnections, websocketMessagesSent, websocketMessagesReceived } = require('./monitoringService');
...
class SocketService {
    ...
    try {
      ...
      results.forEach(result => {
        if (result && result.data.status === 'success') {
          ...
          websocketMessagesSent.inc({ event: 'market-data' });
        }
      });
  ...
  async broadcastTradingSignals() {
    try {
      ...
      websocketMessagesSent.inc({ event: 'trading-signals' });
  ...
  async broadcastMarketStatus() {
    try {
      ...
      websocketMessagesSent.inc({ event: 'market-status' });
